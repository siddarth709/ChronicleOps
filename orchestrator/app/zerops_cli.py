...
async def stop_service(service_id: str, project_id: str) -> None:
    if not project_id:
        logger.error("Cannot stop service: project_id is empty")
        return
    if service_id and len(service_id) >= 20:
        cmd = f"zcli service stop -S {shlex.quote(service_id)} -P {shlex.quote(project_id)}"
    else:
        cmd = f"zcli service stop {shlex.quote(service_id or 'app')} -P {shlex.quote(project_id)}"
    logger.info("running: %s", cmd)
    await _run(cmd)

async def start_service(service_id: str, project_id: str) -> None:
    if not project_id:
        logger.error("Cannot start service: project_id is empty")
        return
    if service_id and len(service_id) >= 20:
        cmd = f"zcli service start -S {shlex.quote(service_id)} -P {shlex.quote(project_id)}"
    else:
        cmd = f"zcli service start {shlex.quote(service_id or 'app')} -P {shlex.quote(project_id)}"
    logger.info("running: %s", cmd)
    await _run(cmd)
